...
class DarwinCoreArchiveLoader
  ...
  def populate(collection)
    ...
    gbif_to_mongo = -> (o) {{
      :loc     => {
        :type => 'Point',
        :coordinates => [o['decimalLongitude'].to_f, o['decimalLatitude'].to_f ] },
      :key     => o['taxonKey'],
      :year    => o['year'],
    ...
    bad_records = -> (o) { 
      o.values_at('decimalLongitude', 'decimalLatitude', 'taxonKey', 'year', 'datasetKey', 'species').any? { |v| v.empty? }
    }
    ...
    Zip::File.open(@archive) do |zip_file|
      ...
      entry.get_input_stream do |io|
        ...
        tsv.reject(&bad_records).map(&gbif_to_mongo).each_slice(1000) {|r| collection.insert r }
      end
    end
  end
end
